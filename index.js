 ...
 * Upload this file to a GitHub repo (e.g. snowflake-test), connect that repo
 * in Settings 
 GitHub, create an event with "Run analysis immediately" on,
 * and watch the investigation feed. Expected findings:
 *
 *   Bug A 
 TypeError: user is null            (products.js caller)
 *   Bug B 
 ReferenceError: price is not defined
 *   Bug C 
 NaN result in the discount helper
 */
...
export function getDisplayName(user) {
  ...
  // of null (reading 'name')".
  if (!user) {
    return 'Unknown User'; // Placeholder as per instruction
  }
  return `${user.name} (${user.email})`
}
...
────────────────────────────────────
export function checkoutTotal(cart) {
  // Intentional bug: `price` is never declared 
 ReferenceError.
  let total = 0
  for (const item of cart) {
    total += item.quantity * price // <-- ReferenceError: price is not defined
...
──────────────────────
export function applyDiscount(amount, discountPercent) {
  // Intentional bug: `settings.taxRate` does not exist 
 undefined 
 the
  // discount math silently produces NaN instead of throwing.
  const settings = { tax: 0.05 }
...
// Self-check 
 run `node index.js` to see every bug fire.
import { pathToFileURL } from 'node:url'

if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  try {
    getDisplayName(null) // throws 
 "uncaught"
  } catch (e) {
    console.error('Bug A confirmed:', e.message)
  }
  try {
    checkoutTotal([{ quantity: 2 }]) // throws 
 ReferenceError
  } catch (e) {
    console.error('Bug B confirmed:', e.message)
  }
